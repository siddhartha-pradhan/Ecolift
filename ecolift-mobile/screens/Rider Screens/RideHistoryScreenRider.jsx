...
import RideHistoryCardChildRider from "./RideHistoryCardChildRider";

const { width } = Dimensions.get("window");

const RideHistoryScreenRider = () => {
  ...
  useEffect(() => {
    const fetchRideHistoryDriver = async () => {
      try {
        const response = await RideService.getRideHistoryForDriver(
          userData.user._id
        );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.contentContainer}>
        <MaterialCommunityIcons name="history" size={200} color="#008080" />
          ...
          <ScrollView contentContainerStyle={{}}>
            {rideHistory && rideHistory.length === 0 ? (
              ...
              rideHistory.map((val, index) => {
                return (
                  <View>
                    <RideHistoryCardChildRider ride={val} index={index} />
                  </View>
                );
              })
            )}
