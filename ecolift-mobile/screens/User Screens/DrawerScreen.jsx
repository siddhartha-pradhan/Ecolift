...
import RideService from "../../services/RideService"; // Assuming you have a RideService for ride-related API calls
...
const DrawerScreen = ({ isVisible, onClose, slideAnim, shadowAnim }) => {
  ...
  // Using AsyncStorage implementation from code 1
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        console.error("Failed to load user data from storage:", error.message);
      }
    };
  ...
  const  cancelAllRides = async (userId) => {
        try {
            const response = await api.post(`/ride/all-cancel`,{id:userId});
  ...
  // Using AsyncStorage logout logic from code 1
  const handlePress = async (label, screen) => {
    setActiveItem(label);

    // const userId = await AsyncStorage.getItem("userId");
    // await cancelAllRides(userId); // Cancel all rides on logout

    if (label === "Logout") {
      AsyncStorage.removeItem("token"); // Clear token on logout
      AsyncStorage.removeItem("userName"); // Clear userName from code 1
      AsyncStorage.removeItem("userEmail"); // Clear userEmail from code 1
      navigation.reset({
        index: 0,
        // @ts-ignore
